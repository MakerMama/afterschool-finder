...
try:
    ...
    with st.form(key='program_filter_form'):
        ...
        with col2:
            ...
            grade_options = ['Not sure', '3K', 'UPK', 'K']
            grade_level = st.selectbox(
                "Child's Grade Level",
                options=grade_options,
                index=grade_options.index(st.session_state.grade_level) if st.session_state.grade_level in grade_options else 3,
                help="Select grade level for on-site school programs (optional)"
            )

        # Program Location filter as checkboxes
        st.markdown('<div style="font-size: var(--font-size-large); font-weight: 600; color: var(--primary-color); margin: 1.5rem 0 0.75rem 0; border-bottom: 2px solid var(--border-color); padding-bottom: 0.5rem;">📍 Program Location <span style="display: inline-block; margin-left: 6px; color: #94a3b8; font-size: 0.9rem; cursor: help;" title="On-site: School-based programs with pickup from school • Off-site: External venues (separate transportation needed)">ⓘ</span></div>', unsafe_allow_html=True)
